...
class ModemHeartbeat(Thread) :
    ...
    def run(self):
        ...
        while not cmd.stop:
            ...
            start = time.time()
            heartbeat_event.wait(stg.heartbeat + mdm.interval)
            finish = time.time()
            
            #-- has the heartbeat timed out?----------------------------
            if finish - start > stg.heartbeat + mdm.interval:
                
                # heartbeat has timed out waiting for modem-------------
                # kill the thread and start a new thread----------------
                
                modem_thread.kill()
                logger.info(F"Heartbeat killed modem thread {mdm.modem_id} {stg.heartbeat}")
        ...
        while not cmd.stop:
            try:
                ...
                if diff < mdm.interval - 0.001:
                    display_event.clear()
                    display_event.wait(timeout = mdm.interval - 0.001 - diff)
            except  Exception as e:
...
try:
    if len(sys.argv) == 2:
        if sys.argv[1] == '-a' or sys.argv[1] == '--about':
            if platform.system() == 'Linux':
                subprocess.run(['./slit','README.md'])
            elif platform.system() == 'Windows':
                subprocess.call('.\slit.exe README.md')
